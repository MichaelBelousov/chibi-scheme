...
js/chibi.wasm: chibi-scheme-emscripten chibi-scheme-static.bc js/pre.js js/post.js js/exported_functions.json
	emcc -O0 chibi-scheme-static.bc -o js/chibi.wasm -s ALLOW_MEMORY_GROWTH=1 -s SIDE_MODULE=1 -s MODULARIZE=1 -s EXPORT_NAME=\"Chibi\" `find  lib -type f \( -name "*.scm" -or -name "*.sld" \) -printf " --preload-file %p"` -s 'EXPORTED_RUNTIME_METHODS=["ccall", "cwrap"]' --pre-js js/pre.js --post-js js/post.js
